...
namespace Spider.Infrastructure.Utils
{
    public static class PropertyUtil
    {
        ...
        /// <returns></returns>
        public static string GetAttributeProperty<T>(T data, Type attributeType)
        {
            var properties = GetTypeProperties(data.GetType());

            var property = properties.Where(m =>
                    {
                        object[] attrs = m.GetCustomAttributes(true);
                        for (int i = 0; i < attrs.Count(); i++)
                        {
                            var exsiteAttribute = m.GetCustomAttribute(attributeType, true);
                            if (exsiteAttribute != null)
                            {
                                return true;
                            }
                        }
            ...
            return property.FirstOrDefault()?.Name;
        }
    }
}
