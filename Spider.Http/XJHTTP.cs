    ...
    public class XJHTTP
    {
        ...
        public bool DonwnLoad(string url, string paths, CookieContainer cc, string defaultName = ".zip")
        {
            try
            {
                ...
                hr = http.AddHttpItem(item).GetHtml();
        ...
        {
            item.Url = url;
            return http.AddHttpItem(item).GetHtml();
        }
        /// <summary>
        /// 普通请求.直接返回标准结果
        /// </summary>
        ...
        public Image GetImageByImage(string picurl, string referer, CookieContainer cc)
        {
            ...
            item.Container = cc;
            item.ResultType = ResultType.Byte;
            return http.GetImg(http.AddHttpItem(item).GetHtml());
        }
        /// <summary>
        /// 针对于Base64形式的图片/验证码
        /// </summary>
        ...
        {
            return await http.AddHttpItem(objHttpItems).GetHtmlAsync();
        }
        /// <summary>
        /// 异步GET请求 通过回调返回结果
        /// </summary>
